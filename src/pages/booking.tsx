...
import { Mountain, Lock, Printer, ArrowLeft, CheckCircle, CreditCard, Mail } from 'lucide-react';
...
export default function BookingPage() {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-mountain-50 via-white to-sunrise-50/20 relative overflow-hidden">
        {/* Subtle geometric pattern instead of floating emoji */}
        <div className="absolute inset-0 pointer-events-none">
          <AkanPatternBackground opacity={0.02} color="#334155" />
          <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-gradient-to-bl from-forest-100/30 to-transparent rounded-full -translate-y-1/3 translate-x-1/3" />
          <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-gradient-to-tr from-sunrise-100/20 to-transparent rounded-full translate-y-1/4 -translate-x-1/4" />
        </div>
        
        {/* Header */}
        <header className="bg-mountain-900/95 backdrop-blur-xl text-white py-4 sticky top-0 z-50 border-b border-white/5">
              ...
              {[1, 2, 3].map((s) => (
                <div key={s} className="flex items-center">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                    s === step ? 'bg-sunrise-500 text-white shadow-lg shadow-sunrise-500/30' :
                ...
                <div className="bg-white rounded-2xl shadow-sm p-6 border border-mountain-100">
                  ...
                  <div className="bg-mountain-50 rounded-xl p-4 mb-6">
                    <div className="flex items-start gap-3">
                      <Lock className="w-5 h-5 text-mountain-600 flex-shrink-0 mt-0.5" />
                      <div>
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      {isProcessing ? (
                        <>
                          <span className="animate-spin inline-block"><CreditCard className="w-5 h-5" /></span>
                          Processing...
                        </>
                      ) : (
                        <>
              ...
              {step === 3 && (
                <div className="bg-white rounded-2xl shadow-sm p-8 border border-mountain-100 text-center">
                  ...
                  <div className="bg-forest-50 rounded-xl p-4 mb-8 text-left max-w-md mx-auto">
                    <p className="text-forest-800 text-sm flex items-start gap-2">
                      <Mail className="w-4 h-4 flex-shrink-0 mt-0.5" /> A confirmation email has been sent to <strong>{formData.email}</strong>
                    </p>
                  </div>
                  ...
                  <div className="flex gap-3 justify-center">
                    <Link href="/">
                      <button className="px-6 py-3 border-2 border-mountain-200 text-mountain-700 font-bold rounded-xl hover:bg-mountain-50 transition flex items-center gap-2">
                        <ArrowLeft className="w-4 h-4" /> Back to Home
                      </button>
                    </Link>
                    <button 
                      onClick={() => window.print()}
                      className="px-6 py-3 bg-forest-600 text-white font-bold rounded-xl hover:bg-forest-700 transition flex items-center gap-2"
                    >
